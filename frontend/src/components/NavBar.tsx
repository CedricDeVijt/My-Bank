...
import {AUTH_CHANGED_EVENT, ApiError, clearTokens, getCurrentUser, loadTokens} from "../services/auth";
...
export function NavBar() {
    ...
    useEffect(() => {
        ...
        const handleDocumentInteraction = (event: MouseEvent | KeyboardEvent) => {
            ...
            if (event instanceof MouseEvent && menuRef.current && !menuRef.current.contains(event.target as Node)) {
                setMenuOpen(false);
            }
        };
                    ...
                    <NavLink
                        ...
                        className={({isActive}) =>
                            `rounded-full px-4 py-2 text-sm font-medium transition ${
                                isActive ? "bg-amber-300 text-slate-950" : "text-slate-200 hover:bg-white/5 hover:text-white"
                            }`
                        }
                    >
                        Login
